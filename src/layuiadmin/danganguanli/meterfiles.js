layui
  ...
  .use(["index", "table", "layer"], function() {
    ...
    form.on("submit(IccRegist-search)", function(data) {
      ...
      console.log(data, "数据");
      //执行重载
      table.reload("meterFilesTable", {
        where: field
      });
    });

    //用户管理表格渲染
    var meterFilesTable = table.render({
      elem: "#meterFilesTable",
    ...
    //监听复选框
    table.on("checkbox(table-MeterFiles)", function(obj) {
    ...
    //监听工具条
    table.on("tool(table-MeterFiles)", function(obj) {
      var data = obj.data;
